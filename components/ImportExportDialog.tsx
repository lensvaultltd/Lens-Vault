                ...
                <Tabs defaultValue="import" className="space-y-4">
                    ...
                    <TabsContent value="import" className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-4 hover:border-primary transition-colors">
                                <input
                                    type="file"
                                    accept=".csv,.json,.txt"
                                    onChange={handleFileUpload}
                                    className="w-full cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-primary-foreground hover:file:bg-primary/90"
                                />
                                {uploadedFile && (
                                    <p className="text-sm text-muted-foreground mt-2">📄 Selected: {uploadedFile.name}</p>
                                )}
                            </div>
                            <p className="text-xs text-muted-foreground">Or paste your data below:</p>
                            <Textarea placeholder="Paste your exported data here..." value={importData} onChange={(e) => setImportData(e.target.value)} rows={8} className="font-mono text-sm" />
                        </div>
                        <div className="space-y-2">
