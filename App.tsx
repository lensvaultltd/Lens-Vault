    ...
    <div className="min-h-screen transition-all duration-700">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
          <TabsList className={`${viewingAs ? 'w-full' : 'w-full md:grid md:grid-cols-5'} h-auto p-1 bg-muted/20 backdrop-blur-md rounded-2xl border border-white/10 md:inline-grid overflow-x-auto scrollbar-hide`}>
            <TabsTrigger value="vault" className="gap-2 whitespace-nowrap flex-shrink-0 rounded-xl data-[state=active]:shadow-md transition-all duration-300">
              <Shield className="h-4 w-4" /> {viewingAs ? `Shared Items (${displayedEntries.length})` : 'My Vault'}
            </TabsTrigger>
            {!viewingAs && (
              <>
                <TabsTrigger value="auditor" className="gap-2 whitespace-nowrap flex-shrink-0 rounded-xl data-[state=active]:shadow-md transition-all duration-300">
                  <ShieldAlert className="h-4 w-4" />AI Auditor
                </TabsTrigger>
                <TabsTrigger value="generator" className="gap-2 whitespace-nowrap flex-shrink-0 rounded-xl data-[state=active]:shadow-md transition-all duration-300">
                ...
                {!isPremium ? (
                  ...
                  <TabsTrigger value="authorized" className="gap-2 whitespace-nowrap flex-shrink-0 rounded-xl data-[state=active]:shadow-md transition-all duration-300">
                    <Users className="h-4 w-4" />Family & Sharing
                  </TabsTrigger>
                )}
